...
def mock_llm_client():
    ...
    # 创建 mock decision 对象
    mock_decision = Mock()
    mock_decision.thought = "测试推理"
    mock_decision.action_type = "skip"
    mock_decision.target = None
    mock_decision.confidence = 0.5
    mock_decision.reasoning = "测试理由"
    ...
    mock_decision.content = "测试发言内容"
    mock_decision.speaking_order = "sequential"
...
async def test_werewolf_vote_to_kill(mock_llm_client):
    ...
    # mock 返回 None 是预期的（因为 action_type 是 "skip"）
    assert target_id is None or target_id in [2]
