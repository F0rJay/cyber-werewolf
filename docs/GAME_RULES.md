...
1. **警长竞选**
   - 玩家选择是否竞选警长（上警）
   - 不竞选者不可发言
   - **警上发言顺序**：顺序发言，随机选择第一个发言的玩家，然后按玩家序号顺序发言（循环）
...
4. **发言阶段**
   - **发言顺序规则**：
     - 如果有警长：警长选择顺序或逆序发言，以警长序号为中心，警长为归票位（最后发言）
       - 顺序发言：从警长下一个玩家开始，按序号顺序发言，最后警长发言
       - 逆序发言：从警长前一个玩家开始，按序号逆序发言，最后警长发言
       - 例如：8人局，警长为2号
         - 顺序发言：3 → 4 → 5 → 6 → 7 → 8 → 1 → 2（警长）
         - 逆序发言：1 → 8 → 7 → 6 → 5 → 4 → 3 → 2（警长）
     - 如果没有警长：随机顺序发言
   - 按顺序发言
   - **自爆规则**：
     - 只有狼人可以自爆（与是否警长无关）
...
- **Agent LLM 完整集成**：
  - 发言系统：所有 Agent 支持 LLM 生成发言内容（正常发言、警长竞选发言、警长PK发言）
  - 投票系统：所有 Agent 支持 LLM 决策投票目标（放逐投票、警长投票）
  - 行动决策：所有角色都有 LLM 集成（预言家、女巫、守卫、狼人）
- **完整的技能系统（LLM 决策）**：
  - 女巫解药/毒药完整实现
  - 守卫守护完整实现
  - 预言家查验完整实现
- **狼人频道发言**：狼人可以在天黑时在狼人频道讨论策略（LLM 生成）
- **自爆机制**：狼人可以自爆，终止发言直接进入黑夜（LLM 决策）
